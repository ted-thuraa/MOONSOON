...
class SocialAuthController extends Controller
{
    ...
    public function handlecallback(Request $request)
    {
        try{
                ...
                $user = User::where('authprovider', '=', 'google')
                    ...
                    ->first();
                $userExist_notfromgoogle = User::where('email', '=', $userFromGoogle->email)->first();
            
                ///**
                // */
                if (!$user) {
                    //register
                    if (!$userExist_notfromgoogle){
                        // $isAdmin = $userFromGoogle->email === 'mutwirited@gmail.com'? true : false;
                        $user = User::create([
                            'authprovider' => 'google',
                            ...
                            'is_email_verified' => $userFromGoogle->verifiedEmail,
                        ...
                        //$userExist_notfromgoogle = Auth::login($userExist_notfromgoogle);
                        $token = $userExist_notfromgoogle->createToken('main')->plainTextToken;
                        return response([
                            'user' => $userExist_notfromgoogle,
                            'token' => $token,
                            'email existed not from google tho' 
                        ]);
                    }
    ...
    private function getClient():\Google_Client
    {
         // create the client
         $client = new Client();
         $client->setApplicationName('creatorpage');
         $client->setAuthConfig(storage_path('creatorpageClient_json.json'));
